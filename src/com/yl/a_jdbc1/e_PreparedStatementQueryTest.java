...
import com.yl.utils.bean.Customer;
 ...
 *
 */
public class e_PreparedStatementQueryTest {
		...
		try {
			...
			if (rs.next()) {
				...
				for (int i = 0; i < columnCount; i++) {
					...
					// 获取每个列的列名。下面赋值时，当列名和class字段名对应不上的时候会报错
					// String columnName = rsmd.getColumnName(i + 1);
					// getColumnLabel获取的是列别名，使得获取到的列名可以和class的字段名一一对应。当没有别名时获取的是列名
					String columnLabel = rsmd.getColumnLabel(i + 1);
